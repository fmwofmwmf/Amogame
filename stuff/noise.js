...
noiseSeed(-152)
function noisestuff(x, y, scale, count, index, weights = []) {
    let noi = []
    if (weights.length == 0) {
        weights = new Array(count).fill(1)
    }
    for (let i = 0; i < count; i++) {
        noi.push(noise(x / scale, y / scale, index + i) * weights[i])
    }
    return noi.indexOf(Math.max(...noi))
}